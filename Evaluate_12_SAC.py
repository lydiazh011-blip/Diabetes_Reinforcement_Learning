...
MODEL_DIR = f"sac_models/{PATIENT_NAME}"
...
def _to_scalar_action(action):
    ...
    if a.ndim == 0:
        action_for_env = float(a)
        action_value = float(a)
    else:
        action_for_env = a.astype(np.float32)
        action_value = float(a.flatten()[0])

    return action_for_env, action_value
...
def run_single_episode(cap, seed, show_steps=True):
    ...
    venv = VecNormalize.load(
        f"{MODEL_DIR}/sac_vecnorm_cap_{cap:.2f}.pkl",
        venv
    )
    venv.training = False
    ...
    for step in steps:
        ...
        action_for_env, action_value = _to_scalar_action(action)
        obs, _, done, truncated, _ = env.step(action_for_env)

        cgm = float(obs[0])
        cgm_trace.append(cgm)
        action_trace.append(action_value)

        if PRINT_EVERY and step % PRINT_EVERY == 0:
            tqdm.write(
                f"step={step:4d} | CGM={cgm:6.1f} | action={action_value:.4f}"
            )
...
def plot_cap_vs_tir(results):
    ...
    plt.title(f"Single SAC Performance vs Cap ({PATIENT_NAME})")
    plt.grid(True)
    plt.ylim(0, 1)
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    print("=" * 70)
    print("Single SAC Evaluation + Visualization")
    ...
    for cap in cap_iter:
        model_path = f"{MODEL_DIR}/sac_cap_{cap:.2f}.zip"
        norm_path = f"{MODEL_DIR}/sac_vecnorm_cap_{cap:.2f}.pkl"
